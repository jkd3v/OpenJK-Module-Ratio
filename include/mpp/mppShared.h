...
typedef struct {
	...
	void(*AddCommand)(const char *cmdName, xcommand_t function, const char *cmd_desc);
	...
	qboolean(*GetStringTextString)(const char *text, char *buffer, int bufferLength);
	...
	struct {
		...
		void(*Trace)(trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, int capsule);
		clipHandle_t(*InlineModel)(int index);
		void(*LoadMap)(const char *mapname, qboolean SubBSP, int *checksum);
		...
		clipHandle_t(*TempModel)(const vec3_t mins, const vec3_t maxs, int capsule);
		void(*TransformedTrace)(trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, int capsule);
	...
	struct {
		...
		void(*Register)(vmCvar_t *vmCvar, const char *varName, const char *defaultValue, uint32_t flags);
		void(*Set)(const char *var_name, const char *value, int vmslot);
	...
	struct {
		...
		void(*AddLine)(vec3_t start, vec3_t end, float size1, float size2, float sizeParm, float alpha1, float alpha2, float alphaParm, vec3_t sRGB, vec3_t eRGB, float rgbParm, int killTime, qhandle_t shader, int flags);
		...
		qboolean(*PlayBoltedEffectID)(int id, vec3_t org, void *ghoul2, const int boltNum, const int entNum, const int modelNum, int iLooptime, qboolean isRelative);
	...
	struct {
		...
		void(*StartSound)(const vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx);
